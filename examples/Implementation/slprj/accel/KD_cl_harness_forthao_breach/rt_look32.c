...
int_T idx = 0 ; int_T bottom = 0 ; int_T top = xlen - 1 ; int_T retValue = 0
; boolean_T returnStatus = 0U ;
#ifdef DOINTERPSEARCH     
real32_T offset = 0 ;
#endif
if ( u <= x [ bottom ] ) { retValue = ( bottom ) ; returnStatus = 1U ; } else
if ( u >= x [ top ] ) { retValue = ( top - 1 ) ; returnStatus = 1U ; } else {
} if ( returnStatus == 0U ) { if ( u < 0 ) { for ( ; ; ) {
#ifdef DOINTERPSEARCH
...
#endif
if ( u <= x [ idx ] ) { top = idx - 1 ; } else if ( u > x [ idx + 1 ] ) {
bottom = idx + 1 ; } else { retValue = ( idx ) ; break ; } } } } return
retValue ; }
