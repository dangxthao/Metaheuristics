...
MRS_input_gen = var_cp_signal_gen(input_str, input_cp, input_intp);
...
%MRS_system.SetInputGen(S);
%MRS_system.SetParamGen(pg);
    ...
    if ii < 9 
        ...
        for jj = 0:(nbctrpt-1)
            ...
            if (jj<(nbctrpt-1))
                ...
                if (jj>0) 
                    input_range = [input_range; (jj*sim_time/nbctrpt -eps_time)  (jj*sim_time/nbctrpt + eps_time) ];
                else
                    input_range = [input_range; jj*sim_time/nbctrpt  (jj*sim_time/nbctrpt + eps_time) ];
                end
