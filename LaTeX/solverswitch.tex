...
The exploitation methods make effective use of local information regarding the shape of the cost surface, but they can become stuck near local optima. Exploration methods avoid this problem, but they do make effective use of local information. To achieve an effective method that combines exploitation and exploration techniques, it is important to have appropriate measures for exploitation and exploration performance. Exploitation performance can be measured by the reduction of the robustness value. Exploration performance can be measured using the notion of coverage. In the optimization process, we use only a value coverage measure instead of time pattern coverage, because the time pattern generation is guaranteed to have good coverage. Before describing how we use these measures to guide our metaheuristic approach, we briefly recall the signal value coverage notion.

\paragraph{Signal Value Coverage as Exploration Performance Measure}	

The metaheuristic procedure we describe in the sequel utilizes a notion of signal value coverage to guide the search.
This is a metric called {\em cell occupancy}. Note that our set of signals corresponds to a set $\inputparamset$ of its values.
...
We now describe the $InitialPointSelection$ function in Algorithm \ref{algoSolverExpl}, which selects initial parameter points for a solver. We propose three heuristics to do so:
\begin{itemize}
