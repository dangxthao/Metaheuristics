...
We formulate the property falsification task as an optimization problem, which we then attempt to solve using existing algorithms, such as simulated annealing or gradient descent. Specifically, we apply nonlinear optimization techniques to the following problem.
\begin{eqnarray} \label{eq:optim1}
\min_{\param \in \paramset} && \rho(\spec,y) \\ \nonumber
s.t. && y=\behaviorfunc(\param)
\end{eqnarray}

The cost function in (\ref{eq:optim1}) is not convex and not continuous, and so we do not expect to obtain an optimal answer using existing algorithms.
Instead, we seek to identify an iterative algorithm that can reduce the cost in (\ref{eq:optim1}) so that $\rho(\spec,y)<0$, which corresponds to a counterexample.

In the next section, we describe a method that iteratively selects from a collection of different existing optimization algorithms, each time using information about the progress of the search to decide which of the algorithms to use in the next iteration.
